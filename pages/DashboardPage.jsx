...
const QuickActionButton = ({ title, icon, onClick, description }) => (
    <button 
        onClick={onClick} 
        className="group flex flex-col items-center justify-center space-y-3 bg-white p-6 rounded-xl shadow-sm hover:shadow-lg border border-gray-100 text-gray-700 hover:border-emerald-200 transition-all duration-300 transform hover:-translate-y-1 min-h-[140px]"
    >
        <div className="p-4 bg-emerald-50 rounded-xl text-emerald-600 group-hover:bg-emerald-100 group-hover:scale-110 transition-all duration-300">
            {icon}
        </div>
        <div className="text-center">
            <span className="font-semibold text-sm sm:text-base block mb-1 group-hover:text-emerald-600 transition-colors">{title}</span>
            {description && <span className="text-xs text-gray-500 hidden sm:block">{description}</span>}
        </div>
    </button>
)
...
const WelcomeSection = ({ userName }) => {
    ...
    return (
        <div className="bg-gradient-to-br from-emerald-50 via-white to-blue-50 rounded-2xl p-6 sm:p-8 mb-8 border border-emerald-100">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-gray-600 text-base">
                        Monitor your livestock and manage your farm efficiently
                    </p>
                </div>
                <div className="mt-4 sm:mt-0">
                    <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-emerald-100">
...
const DashboardPage = () => {
    ...
    const stats = useMemo(() => {
        if (!history || history.length === 0) {
            return { totalAnalyses: 0, mostCommonBreed: 'No data yet' };
        }
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
                ...
                <section className="mb-12">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6">
                        <StatCard title="Total Analyses" value={stats.totalAnalyses} icon={<TotalAnalysisIcon />} />
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-4 sm:space-y-0">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full sm:w-80 pl-10 pr-4 py-2 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                            />
